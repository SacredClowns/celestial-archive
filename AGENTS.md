# AGENTS.md — Codex's Operating Rules for the Celestial Archive
...
## HOW Codex COLLABORATES WITH GPT AND CURSOR

```
Codex writes → Markdown content files with YAML frontmatter
                 ↓
...
**The boundary:** Codex writes the *what* and *why*. GPT builds the *how*. Cursor executes the *build*. Human approves the *whether*.

### Handoff format for content:

Every content file Codex produces must include:
...
- Codex submits the lesson + source pack + verification pass
